...
import os, numbers, glob, shutil, inspect, copy
import numpy as np
...
class ModelHelper(object):
    ...
    def __init__(self, model, root_name, ids, gen_params={}, verbose=False, **params):
        """
        ...
        * verbose:    verbosity
        ...
        self.model_name = ShortNameBuilder(prefix=f"{root_name}/", sep=(":", " "))
        self.model_cpu = None

        self.gen_params = Munch(
            shuffle=True,
            process_fn=False,
            deterministic=False,
            verbose=verbose,
        ...
        for key in list(params.keys()):
            if key not in list(self.params.keys()):
                raise Exception(f"Undefined parameter:{key}")
        ...
        if self.params.gen_class is None:
            if getattr(self.gen_params, "data_path", None) is not None:
                self.params.gen_class = (
                    DataGeneratorHDF5
                    if self.gen_params.data_path[-3:] == ".h5"
                    else DataGeneratorDisk
                )
    ...
    def set_callbacks(self):
        ...
        best_model_path = os.path.join(
            p.models_root, f"{self.model_name()}_best_weights.h5"
        )
    ...
    def make_generator(self, ids, subset=None, **kwargs):
        ...
        * kwargs: updated parameters of the generator
        ...
        ids_ = ids[ids.set == subset] if subset else ids
    ...
    def train(
        ...
        * epochs:    number of epochs
        ...
        if valid_gen is True:
            valid_gen = self.make_generator(
                ids[ids.set == "validation"], shuffle=True, deterministic=True
            )

        if recompile:
            if lr:
                self.params.lr = lr
            self.params.optimizer = update_config(
                self.params.optimizer, lr=self.params.lr
            )
            self.compile()
    ...
    def predict(
        self, test_gen=None, output_layer=None, repeats=1, batch_size=None, remodel=True
    ):
        """
        Predict on `test_gen`.
        ...
        * remodel:      if `true` then change model such that new output is `output_layer`
        ...
        """
        if not test_gen:
            params_test = self._updated_gen_params(shuffle=False, fixed_batches=False)
            if batch_size:
                params_test.batch_size = batch_size
            test_gen = self.params.gen_class(
                self.ids[self.ids.set == "test"], **params_test
            )
        if output_layer is not None and remodel:
            # get last partial-matching layer
            layer_name = [l.name for l in self.model.layers if output_layer in l.name][
                -1
            ]
            ...
            model = Model(inputs=self.model.input, outputs=outputs)
        else:
            model = self.model
    ...
    def validate(self, valid_gen=True, verbose=2, batch_size=32, recompile=True):
        if valid_gen is True:
            ids = self.ids
            valid_gen = self.make_generator(
                ids[ids.set == "validation"], shuffle=True, deterministic=True
        ...
        if issubclass(type(valid_gen), keras.utils.Sequence):
            r = self.model.evaluate(valid_gen, verbose=verbose)
        else:
            X_valid, y_valid = valid_gen
            r = self.model.evaluate(
                X_valid, y_valid, batch_size=batch_size, verbose=verbose
            )
        perf_metrics = dict(list(zip(self.model.metrics_names, force_list(r))))
        if verbose == 2:
            pretty(
                OrderedDict((k, perf_metrics[k]) for k in sorted(perf_metrics.keys()))
            )
        return perf_metrics
    ...
    def load_model(
        self, model_name="", best=True, from_weights=True, by_name=False, verbose=1
    ):
        """
        ...
        * by_name:      load layers by name
        ...
        model_file_name = (
            model_name
            + ("_best" if best else "_final")
            + ("_weights" if from_weights else "")
            + ".h5"
        )
    ...
    def save_model(
        self, weights_only=False, model=None, name_extras="", best=False, verbose=1
    ):
        """
        ...
        * best:         save as best model, otherwise final model
        ...
        if verbose:
            print(
                "Saving model", model.name, "spanning", len(self.model.layers), "layers"
            )
        model_type = "best" if best else "final"
        if weights_only:
            model_file = (
                self.model_name() + name_extras + "_" + model_type + "_weights.h5"
            )
    ...
    def save_activations(
        ...
        * postprocess_fn: post-processing function to apply to the activations
        ...
        if file_path is None:
            ...
            short_name(grp=groups_count, lay=(output_layer or "final"))
            file_path = os.path.join(
                self.params.features_root, str(short_name) + name_suffix + ".h5"
            )
            make_dirs(file_path)

        params = self._updated_gen_params(
            shuffle=False, verbose=verbose, fixed_batches=False, group_by=None
        )
        if verbose > 1:
        ...
        for group_name in groups_list:
            ...
            with H5Helper(file_path, overwrite=overwrite, verbose=verbose) as h:
                if groups == 1:
                    h.write_data(activ, list(ids.loc[:, data_gen.inputs[0]]))
                else:
                    h.write_data(
                        [activ],
                        list(ids.loc[:, data_gen.inputs[0]]),
                        group_names=[group_name],
                    )
            del activ

    def save_features(
        self, process_gen, batch_size=1024, save_as_type=np.float32, overwrite=False
    ):
        """
