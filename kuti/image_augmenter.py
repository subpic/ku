...
class ImageAugmenter(object):
    ...
    def crop(self, crop_size, crop_pos=None, clip_rotation=False):
        ...
        * clip_rotation: clip a border around the image, such that the edge resulting from
        ...
        crop_size = [
            np.int32(c * dim) if 0 < c <= (1 + 1e-6) else c
        ...
        if crop_size != self.image.shape[:2]:
            if crop_pos is None:
                if clip_rotation:
                    ...
                    border = (0, 0)
                self.image = extract_random_patch(
                    self.image, patch_size=crop_size, border=border
                )
            else:
                self.image = extract_patch(
                    self.image, patch_size=crop_size, patch_position=crop_pos
                )
        return self
    ...
    def cropout(self, crop_size, crop_pos=None, fill_val=0):
        ...
        * fill_val:  value to fill the cropout with
        ...
        crop_size = [
            np.int32(c * dim) if isinstance(c, float) and c > 0 and c <= 1.0 else c
            for c, dim in zip(crop_size, self.image.shape[:2])
        ]
        ...
        if crop_size != self.image.shape[:2]:
            if crop_pos is None:
                border = (0, 0)
                self.image = cropout_random_patch(
                    self.image, patch_size=crop_size, border=border, fill_val=fill_val
                )
            else:
                self.image = cropout_patch(
                    self.image,
        ...
        * min_dim: bool
        ...
        if isinstance(target, int):  # target dimensions
            zoom_target = (
                ...
                if min_dim
                else self.image.shape[0]
            )
            zoom = old_div(1.0 * target, zoom_target)
        else:
            zoom = target
        zoom = (1 - proportion) + proportion * zoom

        self.image = transform.rescale(
            self.image, zoom, preserve_range=True, multichannel=True, mode="reflect"
        )
        return self
        ...
        * fit_frame: size concerns the dimensions of the frame that the image is to be
        ...
        if not fit_frame:
            ...
            if (not ensure_min and size != imsz) or (
                ...
                self.image = transform.resize(self.image, size, preserve_range=True)
        else:
            ...
            target_width, target_height = (
                int(np.round(target_width)),
                int(np.round(target_height)),
            )

            self.image = transform.resize(
                self.image, (target_height, target_width), preserve_range=True
            )
        return self
    ...
    def result(self):
        ...
        """
        return mapmm(self.image, self._original_range) if self._remap else self.image
...
def get_patch_dims(im, patch_size, patch_position):
    ...
    * patch_position: 2-tuple containing patch location
    ...
    X0, Y0 = min(max(int(X0), 0), X_max), min(max(int(Y0), 0), Y_max)

    return (X0, Y0, X0 + W_crop, Y0 + H_crop)


def extract_patch(im, patch_size=(224, 224), patch_position=(0.5, 0.5)):
    """
...
def cropout_patch(im, patch_size=(224, 224), patch_position=(0.5, 0.5), fill_val=0):
    """
