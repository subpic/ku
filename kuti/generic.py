...
class H5Helper(object):
        ...
        * verbose: verbosity
        ...
        with self._lock:
            if memory_mapped:
                ...
                self.hf = h5py.File(
                    file_name, _file_mode, driver="core", backing_store=backing_store
                )
            else:
    ...
    def write_data(self, data, dataset_names, group_names=None):
        ...
        * group_names:   None, or list of strings
        ...
        with self._lock:
            assert isinstance(
                dataset_names, list
            ), "`dataset_names` is of type {} and should be `list`".format(
                type(dataset_names)
            )
            ...
            if group_names is None:
                ...
                assert isinstance(data, list) and len(data) == len(
                    group_names
                ), "Each group name should correspond to a `data` list entry, `len(data)`={}, while `len(grou_names)`={}".format(
                    len(data), len(group_names)
                )
    ...
    def _read_datasets(self, reader, dataset_names):
        ...
        data0 = reader[name][...]
        data = np.empty((len(dataset_names),) + data0.shape, dtype=data0.dtype)
        data[0, ...] = data0
        ...
        * group_names:   None, or list of strings
        ...
        with self._lock:
            ...
            if group_names is None:
                ...
                return [
                    self._read_datasets(hf[group_name], dataset_names)
    ...
    def read_data_random_group(self, dataset_names):
        ...
        * dataset_names: list of strings
        ...
        with self._lock:
            ...
            names = [
                "{}/{}".format(g, d) for g, d in zip(group_names[idx], dataset_names)
            ]
...
def dist2mos(x, scale=np.arange(1, 6)):
    ...
    """
    x = old_div(x, np.reshape(np.sum(x * 1.0, axis=1), (len(x), 1)))
...
def updated_dict(d, only_existing=True, **updates):
    ...
    * updates: dict
    ...
    if only_existing:
        common = {
            key: value for key, value in list(updates.items()) if key in list(d.keys())
        }
        d.update(common)
    else:
        d.update(updates)
...
class ShortNameBuilder(Munch):
    ...
    def __init__(self, prefix="", sep=("", "_"), max_len=32, **kwargs):
    ...
    def subset(self, selected_keys):
        ...
        for k in list(subself.keys()):
            if k not in selected_keys and "_ShortNameBuilder" not in k:
                del subself[k]
        return subself
    ...
    def __str__(self):
        ...
        return self.__prefix + self.__sep[1].join(
            [
                combine(k, self[k])
...
def partition_rows(
    ...
    itrain, itest = train_test_split(
        list(range(len(t))), test_size=test_size, random_state=random_state
    )
...
def download_archive(
    ...
    if archive_type == "zip":
        failed_command = subprocess.run(
            ["unzip", "-qq", "-o", data_path, "-d", data_root], check=True
        )
    else:
        failed_command = subprocess.run(
            ["tar", "-xf", data_path, "-C", data_root], check=True
        )
