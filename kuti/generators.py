...
class DataGeneratorDisk(keras.utils.Sequence):
    ...
    * random_group (bool): read inputs from a random group for every image
    ...
    def __init__(self, ids, data_path, **args):
        ...
        params.deterministic = {True: 42, False: None}.get(
            params.deterministic, params.deterministic
        )
    ...
    def __len__(self):
        ...
        return int(round_op(len(self.ids) * 1.0 / self.batch_size))
    ...
    def __getitem__(self, index):
        ...
        if self.shuffle:
            ids_batch = ids_batch.reset_index(drop=True).sample(
                frac=1, random_state=self.deterministic
            )
    ...
    def on_epoch_end(self):
        ...
        group_dict = dict(group_by=self.ids[self.group_by]) if self.group_by else None
        self.ids_index = pd.DataFrame(group_dict, index=self.ids.index.copy())
        ...
        while selectable.sum():
            ...
            if self.group_by:
                group_by_value = ids_sel.group_by.sample(
                    1, random_state=self.deterministic
                ).values[0]
                ids_sel = ids_sel[ids_sel.group_by == group_by_value]

            batch_size_max = min(self.batch_size, len(ids_sel))
            if self.shuffle:
                ids_batch = ids_sel.sample(
                    batch_size_max, random_state=self.deterministic
                )
            else:
    ...
    def _read_data(self, ids_batch, accessor):
        X = []
        if accessor:
            assert isinstance(accessor, list) or callable(
                accessor
            ...
            if callable(accessor):
                ...
                raise ValueError("Wrong generator input/output specifications")
                ...
                for input_name in params.inputs:
                    ...
                    for row in ids_batch.itertuples():
                        input_data = os.path.join(
                            group_name, subgroup_name, getattr(row, input_name)
                        )
                    ...
                    if params.process_fn not in [None, False]:
                        ...
                        for i, row in enumerate(ids_batch.itertuples()):
                            ...
                            if args_name is not None:
                                args = [
                                    getattr(row, name) for name in force_list(args_name)
                                ]
...
class DataGeneratorHDF5(DataGeneratorDisk):
    ...
    * random_group (bool): read inputs from a random group for every data instance
    ...
    def _data_generation(self, ids_batch):
        ...
        if isinstance(params.data_path, string_types):
            with H5Helper(
                params.data_path, file_mode="r", memory_mapped=params.memory_mapped
            ) as h:
                # group_names are randomly sampled from meta-groups
                # i.e. when group_names = [[group_names1], [group_names2]]
                group_names = params.group_names
                        ...
                        if params.process_fn not in [None, False]:
                            ...
                            for i, row in enumerate(ids_batch.itertuples()):
                                arg = (
                                    ...
                                    else [getattr(row, args_name)]
                                )
                                data_i = params.process_fn(data[i, ...], *arg)
                                if data_new is None:
                                    data_new = np.zeros(
                                        (len(data),) + data_i.shape, dtype=np.float32
                                    )
                                data_new[i, ...] = data_i
